...
# qa_l_knots[0] = [-np.pi / 6, -np.pi / 6]
# qa_l_knots[1] = [-np.pi / 6, -np.pi / 6]
# q_robot_l_traj = PiecewisePolynomial.FirstOrderHold([0, T * h], qa_l_knots.T)

# qa_r_knots = np.zeros((2, nq_a))
# qa_r_knots[0] = [np.pi / 6, np.pi / 6]
# qa_r_knots[1] = [np.pi / 6, np.pi / 6]
# q_robot_r_traj = PiecewisePolynomial.FirstOrderHold([0, T * h], qa_r_knots.T)

qa_l_knots = np.zeros((2, nq_a))
qa_l_knots[0] = [2.6362174, -0.02206816]
qa_l_knots[1] = [-np.pi / 6, -np.pi / 6]
q_robot_l_traj = PiecewisePolynomial.ZeroOrderHold([0, T * h], qa_l_knots.T)

qa_r_knots = np.zeros((2, nq_a))
qa_r_knots[0] = [0.9469534, -0.08627483]
qa_r_knots[1] = [np.pi / 6, np.pi / 6]
...
q_u0 = np.array([-0.10213429, 0.31937248, 0.00655218])
# q_u0 = np.array([0, 0.3, 0])
